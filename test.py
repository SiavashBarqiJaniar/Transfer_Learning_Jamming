...
try:
    ...
    #sourceModel.summary()
    if flagRL:
        targetModel = load_model(f'RL{scenario}.h5')
    elif flagTLwinit:
        targetModel = load_model('DNN.h5')
...
def jamDist():
    ...
    jammingDistribution = [.15, .15, .1, .1, .1, .1, .1, .1, .1]
...
for timeSlot in range(1, noSimulationTimeSlots + 1):
    ...
    if timeSlot % t2 == 0:
        ...
        if flagTLfreeze:
            dfCurrentThruput.to_csv(f'{scenario}_testThroughputsTLfreeze.csv', index=False)
        elif flagTLclassifier:
            dfCurrentThruput.to_csv(f'{scenario}_testThroughputsTLclassifier.csv', index=False)
        elif flagTLwinit:
            dfCurrentThruput.to_csv(f'{scenario}_testThroughputsTLwinit.csv', index=False)
        else:
            dfCurrentThruput.to_csv(f'{scenario}_testThroughputsRL.csv', index=False)
        dfStates = pd.DataFrame( np.concatenate(bufferStateAfterSendingSignal) )
        dfStates.to_csv(f'{scenario}_testStates.csv', index=False)
        dfQ = pd.DataFrame(QBuffer)
        dfQ.to_csv('Qvalues.csv')
        dfActions = pd.DataFrame( bufferActionAfterSendingSignal )
        dfActions.to_csv(f'{scenario}_testActions.csv', index=False)
